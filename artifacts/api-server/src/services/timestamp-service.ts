...
export class TimestampService {
  ...
  private static readonly serverStartTime = Date.now();
  private static timeOffset = 0;
  private static isNTPSynced = false;
   ...
   */
  static getCurrentTimestampISO(): string {
   ...
   */
  static createOperationTimestamp(): Date {
   ...
   */
  static createScanTimestamp(): Date {
    return this.getCurrentTimestamp();
  }
   ...
   */
  static createAuditTimestamp(): Date {
    return this.getCurrentTimestamp();
  }
   ...
   */
  static async syncWithNTP(ntpServer: string = "pool.ntp.org"): Promise<boolean> {
    try {
      ...
      if (Math.abs(drift) > 1000) {
        ...
        console.log(
          `[TimestampService] Time synchronized. Offset: ${drift}ms, Server time: ${serverTime.toISOString()}`
        );
        return true;
      }
   ...
   */
  static getTimestampDifference(start: Date, end: Date): number {
   ...
   */
  static formatTimestamp(date: Date, format: string = "ISO"): string {
   ...
   */
  static validateTimestamps(record: any): {
    valid: boolean;
    issues: string[];
  } {
    const issues: string[] = [];
  ...
  static async initialize(): Promise<void> {
    ...
    console.log(
      `[TimestampService] Ready. Current time: ${this.getCurrentTimestampISO()}`
    );
  }
}

// Auto-initialize on import
TimestampService.initialize().catch((err) =>
  console.error("[TimestampService] Initialization error:", err)
);
