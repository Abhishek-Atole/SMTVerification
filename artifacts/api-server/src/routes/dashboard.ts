...
import { eq, sql, and, gte, between } from "drizzle-orm";
...
router.get("/dashboard/time-analysis", async (req, res) => {
  try {
    ...
    const hourlyMap = new Map<
      number,
      { hour: number; scanCount: number; passCount: number; defectCount: number }
    >();
...
router.get("/dashboard/feeder-analysis", async (req, res) => {
  try {
    ...
    const feederMap = new Map<
      string,
      { feederNumber: string; scanCount: number; passCount: number; defectCount: number; partNumbers: Set<string> }
    >();
...
router.get("/dashboard/component-analysis", async (req, res) => {
  try {
    ...
    const componentMap = new Map<
      string,
      { partNumber: string; description: string | null; scanCount: number; passCount: number; defectCount: number }
    >();
