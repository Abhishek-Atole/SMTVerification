...
router.post("/sessions/:sessionId/scans", async (req, res) => {
    try {
        ...
        let fuzzyMatchResult = { score: 0, matches: false };
                ...
                if (normalizedMpnId) {
                    ...
                    if (internalIdType === "mpn") {
                        ...
                        if (hasExpectedMpn && expectedMpn) {
                            fuzzyMatchResult = ValidationService.fuzzyMatchValue(normalizedMpnId, expectedMpn, 0.95 // 95% threshold
                            );
                        ...
                        if (!userInputMatches && hasExpectedPartNumber && expectedPartNumber) {
                            fuzzyMatchResult = ValidationService.fuzzyMatchValue(normalizedMpnId, expectedPartNumber, 0.95 // 95% threshold
                            );
                            if (fuzzyMatchResult.matches) {
                                userInputMatches = true;
                        ...
                        if (hasExpectedInternalId && expectedInternalId) {
                            ...
                            fuzzyMatchResult = ValidationService.fuzzyMatchValue(normalizedMpnId, expectedInternalId, 0.95 // 95% threshold
                            );
